...
> 虽然抽象并非没有缺点（例如，糟糕的抽象可能包含过多不重要细节或遗漏重要细节，某些抽象可能影响性能[4](https://www.easemob.com/news/7417)；过于抽象的概念也可能难以理解和形象化[6](https://www.sohu.com/a/225702765_411746)），但在许多情况下，不进行抽象是不可行的，或者至少会极大地限制我们的能力：

- **无法深入理解事物**：如果缺乏抽象，我们将停留在对事物表面现象的感知，无法理解其内在的本质和规律[3](https://baike.baidu.com/item/抽象/9021828)。
- **难以应对复杂性**：面对复杂系统或问题时，如果不进行抽象来简化和分层，我们将很快被海量的细节所淹没，无法有效思考和行动。
- **软件开发将变得混乱且难以维护**：如果代码中策略和细节混杂在一起，缺乏清晰的抽象层，那么代码将难以理解、修改和测试。微小的改动都可能引发连锁反应，导致系统脆弱不堪[4](https://www.easemob.com/news/7417)。
- **科学发展受阻**：数学和许多科学领域的发展都依赖于抽象。没有抽象，就无法从具体问题中提炼出普适性的理论和方法，也就无法推动知识的进步[6](https://www.sohu.com/a/225702765_411746)。例如，没有数字这个抽象概念，牧羊人只能用石头与羊一一对应来计数，效率低下且难以进行更复杂的运算[6](https://www.sohu.com/a/225702765_411746)。

**总结来说**，抽象是人类认知和文明发展的基础之一[2](https://www.zhihu.com/question/542948059/answer/2573256342)。它通过简化复杂性、揭示本质、提高效率和促进知识的通用性，使我们能够更有效地理解和改造世界。虽然在具体应用中需要权衡利弊，并设计出"好的"抽象，但完全摒弃抽象，将使我们在面对复杂问题时寸步难行。
...
## Data Structures 和 ADT 有什么异同？ 

| 维度             | **ADT**                                         | **Data Structure**                      |
| ---------------- | ----------------------------------------------- | --------------------------------------- |
| **定位**         | 抽象层：只规定*能做什么*（操作集合 + 语义契约） | 物理层：具体*怎么做*（存储布局 + 算法） |
| **关注点**       | 功能、接口、不变式、外部可观测结果              | 元素组织方式、时间/空间复杂度、工程细节 |
| **信息隐藏**     | 完全不暴露实现；可由多种结构兑现                | 实现细节全部暴露；面向性能权衡          |
| **可替换性**     | 只要满足同一契约即可互换实现                    | 替换会改变性能/内存/并发特性            |
| **面向对象类比** | "接口 / 抽象类"                                 | "具体类 / 字段 + 方法"                  |
| **测试焦点**     | 行为是否符合契约                                | 辅助验证边界、性能、内存对齐等          |
